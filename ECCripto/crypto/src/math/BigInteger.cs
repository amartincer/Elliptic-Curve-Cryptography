			...
			object obj)
		{
			...
			if (biggie == null)
            {
                if (obj is int || obj is long)
                    biggie = new BigInteger(obj.ToString());
                else return false;
            }
...
//			return new BigInteger(b);
        ...
        /// <returns></returns>
        public static BigInteger Random(BigInteger minValue, BigInteger maxValue)
        {
            if (minValue.CompareTo(maxValue) == 1)
                throw new ArgumentOutOfRangeException("The minValue must be lower than maxValue");
            var bytesMinValue = new List<byte>(minValue.ToByteArray());
            var bytesMaxValue = new List<byte>(maxValue.ToByteArray());
            ...
            var listBytes = Gen(new Random(Environment.TickCount), bytesMinValue, bytesMaxValue, /*bytesMinValue.Count - 1*/0);
        ...
        {
            var maxByte = System.Math.Max(bytesMinValue[index], bytesMaxValue[index]);
            var minByte = System.Math.Min(bytesMinValue[index], bytesMaxValue[index]);
            List<byte> res = new List<byte>();
            if (index != bytesMaxValue.Count-1)
                res.Add((byte)r.Next(minByte, maxByte + 1));
            else res.Add((byte)r.Next(minByte, maxByte));
            if (res[res.Count - 1] == maxByte && index != bytesMaxValue.Count-1)
            {
                res.AddRange(Gen(r, bytesMinValue, bytesMaxValue, index + 1));
            }
            else
            {
                byte[] buffer = new byte[bytesMaxValue.Count - index-1];
                r.NextBytes(buffer);
                res.AddRange(buffer);
            }
            return res;
        ...
        public static implicit operator BigInteger(int a)
        {
        ...
        public int Legendre(BigInteger modulus)
        {
            return Legendre(this, modulus);
        }

        private int Legendre(BigInteger value, BigInteger modulus)
        {
            int res = 1;
            if (value.Equals(0))
                res = ((modulus.Equals(1)) ? 1 : 0);
            ...
            else if ((value % 2).Equals(0))
                res = Legendre(2, modulus) * Legendre(value / 2, modulus);
            else
                res = ((value % 4).Equals(3) && (modulus % 4).Equals(3)) ? -Legendre(modulus, value) : Legendre(modulus, value);
            return res;
        }
        ...
        /// <returns></returns>        
        public BigInteger ModSqrt(BigInteger modulus)
        {
            if (Legendre(modulus) != 1)
                return BigInteger.Zero;
            else if (this.Equals(0))
                return 0;
            else if ((modulus % 4).Equals(3))
                return ModPow((modulus + 1) / 4, modulus);
            ...
            while (n.Legendre(modulus) != -1)
                n++;
            BigInteger x = ModPow((s + 1) / 2, modulus);
            BigInteger b = ModPow(s, modulus);
            BigInteger g = n.ModPow(s, modulus);
            int r = e;
            ...
            while (true)
            {
                ...
                for (; m < r; m++)
                {
                    if (t.Equals(1))
                        break;
                    t = t.ModPow(2, modulus);
                }
                if (m == 0)
                    return x;
                BigInteger gs = g.ModPow(((BigInteger)2).Pow(r - m - 1), modulus);
                g = (gs * gs) % modulus;
                x = (x * gs) % modulus;
                b = (b * g) % modulus;
                r = m;
