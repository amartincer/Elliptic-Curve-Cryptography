...
namespace Org.BouncyCastle.Math.EC
{
    ...
    public class FpCurve : ECCurveBase
    {
        ...
        {
            get { return (4 * a.ToBigInteger().Pow(3) + 27 * b.ToBigInteger().Pow(2)).Equals(0); }
        }
        ...
        public static FpCurve RandomFpCurve(CountBitsPrimeNumber countBits)
        {
            var primeOrder = GetRandomPrimeNumber(countBits);
            FpCurve curve=null;
            do
            {
                var aTemp = BigInteger.Random(0, primeOrder);
                var bTemp = BigInteger.Random(0, primeOrder);
                try { curve = new FpCurve(primeOrder, aTemp, bTemp); }
                catch { continue; }
            } while (curve==null || curve.IsSingular);
            return curve;
        }

        private static BigInteger GetRandomPrimeNumber(CountBitsPrimeNumber countBits)
        {
            string text = "";
            switch (countBits)
            {
            ...
            if (lines[index].Contains("x"))
                return new BigInteger(lines[index].Replace("x", ""),16);
            return new BigInteger(lines[index]);            
        }

        private static ECPoint GetRandomBasePoint(FpCurve curve)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public ECPoint GetRandomBig2DPoint()
        {
            BigInteger x, y;
            do
            {
                x = BigInteger.Random(BigInteger.Zero, q);
                y = (x.Pow(3) + a.ToBigInteger() * x + b.ToBigInteger()) % q;

            } while (y.Legendre(q) == -1);
            ECPoint res = new FpPoint(this, new FpFieldElement(q, x), new FpFieldElement(q, y.ModSqrt(q)));
