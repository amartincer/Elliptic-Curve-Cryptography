			...
			try
			{
				...
				if (!AreEqual(cText, outBytes))
				{
					Fail("failed wrap test " + id  + " expected "
						+ Encoding.Default.GetString(Hex.Encode(outBytes)) + " got "
						+ Encoding.Default.GetString(Hex.Encode(cText)));
				}
			}
			...
			try
			{
				...
				if (!AreEqual(pTextBytes, inBytes))
				{
					Fail("failed unwrap test " + id  + " expected "
						+ Encoding.Default.GetString(Hex.Encode(inBytes)) + " got "
						+ Encoding.Default.GetString(Hex.Encode(pTextBytes)));
				}
			}
			catch (Exception e)
			{
