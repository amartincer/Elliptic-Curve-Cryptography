...
namespace Org.BouncyCastle.Tests
{
		...
		: SimpleTest
	{
			...
			byte[]	output)
		{
			...
			{
				Fail("GOST28147 failed decryption - expected " + Encoding.ASCII.GetString(Hex.Encode(input)) + " got " + Encoding.ASCII.GetString(Hex.Encode(bytes)));
			}
		}
			...
			byte[]      output)
		{
			...
			if (!AreEqual(bytes, output))
			{
				Fail("GOST28147 failed encryption - expected " + Encoding.ASCII.GetString(Hex.Encode(output)) + " got " + Encoding.ASCII.GetString(Hex.Encode(bytes)));
			}
