		...
		public override void PerformTest()
		{
			...
			if (!Arrays.AreEqual(sig1a, sig))
			{
				Fail("PSS Sign test expected "
					+ Encoding.Default.GetString(Hex.Encode(sig1a)) + " got "
					+ Encoding.Default.GetString(Hex.Encode(sig)));
			}
...
//				}
			...
			{
				Fail("PSS Sign test expected "
					+ Encoding.Default.GetString(Hex.Encode(sig1b)) + " got "
					+ Encoding.Default.GetString(Hex.Encode(sig)));
			}
...
//				pss = s.getParameters();
			...
			{
				Fail("PSS Sign test expected "
					+ Encoding.Default.GetString(Hex.Encode(sig1c)) + " got "
					+ Encoding.Default.GetString(Hex.Encode(sig)));
			}
