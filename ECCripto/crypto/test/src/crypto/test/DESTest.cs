		...
		: SimpleTest
	{
		...
		public override void PerformTest()
		{
			...
			for (int i = 0; i != k1In.Length; i++)
			{
				...
				{
					Fail("Failed "
						+ "got " + Encoding.ASCII.GetString(Hex.Encode(k1In))
						+ " expected " + Encoding.ASCII.GetString(Hex.Encode(k1Out)));
				}
			}
			...
			for (int i = 0; i != k2In.Length; i++)
			{
				...
				{
					Fail("Failed "
						+ "got " + Encoding.ASCII.GetString(Hex.Encode(k2In))
						+ " expected " + Encoding.ASCII.GetString(Hex.Encode(k2Out)));
				}
			}
		}
	}
